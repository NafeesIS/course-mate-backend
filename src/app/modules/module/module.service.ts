...
const createModuleIntoDB = async (
  ...
  if (course.createdBy !== userId && !user.roles.includes("admin")) {
    throw new AppError(
      ...
      "You can only create modules for your own courses"
    );
  }
...
const updateModuleIntoDB = async (
  id: string,
  payload: Partial<IModule>,
  userId: string
) => {
  ...
  if (course.createdBy !== userId && !user.roles.includes("admin")) {
    throw new AppError(
      httpStatus.FORBIDDEN,
      "You can only update modules for your own courses"
    );
  }
...
const deleteModuleFromDB = async (id: string, userId: string) => {
  ...
  if (course.createdBy !== userId && !user.roles.includes("admin")) {
    throw new AppError(
      httpStatus.FORBIDDEN,
      "You can only delete modules for your own courses"
    );
  }
...
const reorderModulesInDB = async (
  courseId: string,
  moduleOrders: { moduleId: string; moduleNumber: number }[],
  userId: string
) => {
  const user = await UserModel.findOne({ uId: userId });
  if (!user || !user.roles.includes("admin")) {
  ...
  if (course.createdBy !== userId && !user.roles.includes("admin")) {
    throw new AppError(
      httpStatus.FORBIDDEN,
      "You can only reorder modules for your own courses"
    );
  }
