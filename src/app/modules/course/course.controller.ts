...
export const createCourse = catchAsync(
  async (req: SessionRequest, res: Response) => {
    ...
    if (courseData.price) {
      courseData.price = parseFloat(courseData.price); // or use `Number(courseData.price)`
    ...
    if (req.file) {
      courseData.thumbnail = await uploadToGoogleDrive(
        req.file,
        "course-thumbnails"
      );
    }
...
export const updateCourse = catchAsync(
  async (req: SessionRequest, res: Response) => {
    ...
    if (updateData.price) {
      updateData.price = parseFloat(updateData.price); // or use `Number(updateData.price)`
    ...
    if (req.file && req.file.fieldname === "thumbnail") {
      updateData.thumbnail = await uploadToGoogleDrive(
        req.file,
        "course-thumbnails"
      );
    }
...
export const enrollCourse = catchAsync(
  async (req: SessionRequest, res: Response) => {
