    ...
    documents
      .filter(
        doc =>
          ...
          (doc?.eformName === 'Form MGT-7' || doc?.eformName === 'Form MGT-7A') &&
...
export const calculateMGT7DueDate = (
  incorporationDate: string,
  authorizedShareCapital: number,
  documents: TAnnualEFilingForm[]
): TAnnualEFilingFormResponse[] => {
  const formDetails = getFormDetailsByEFormName('Form MGT-7'); //getting form details based on E-form name
  const dateFormats = ['dd/MM/yyyy', 'dd-MM-yyyy', 'MM/dd/yyyy', 'MM-dd-yyyy'];
  ...
  const mgt7FilingInfo =
    ...
    documents
      ...
      .map(doc =>
        // console.log('Inspecting doc.paymentStatus:', doc.paymentStatus), // Inspect the structure
        // console.log('Direct access to normalFee:', doc.paymentStatus.normalFee), // Check if undefined
  ...
  const firstYearInfo = mgt7FilingInfo?.find(info => getYear(info.eventDate) === baseYear);
  // const firstYearFilingDate =
  //   (firstYearInfo?.filingDate &&
  //     parse(firstYearInfo?.filingDate, 'dd/MM/yyyy', new Date()).toDateString()) ||
  //   'Not filed';
  ...
  for (let year = baseYear; year <= currentYear + 1; year++) {
    ...
    if (financialYear !== firstFinancialYear) {
      ...
      const filingInfo = mgt7FilingInfo?.find(info => getYear(info.eventDate) === getYear(agmDate));
      ...
      const dueDate = addDays(agmDate, 60);
      ...
      //   parse(filingInfo?.filingDate, 'dd/MM/yyyy', new Date()).toDateString();
